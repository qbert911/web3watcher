...
def convex_header_display(myarray, carray, w3, fullheader):
    ...
    print(" "*9,"v"+str(format(round(myarray[-1]["trix_rewards"][1],2), '.2f')).rjust(5)+"x"+str(format(round(myarray[-1]["trix_rewards"][2],2), '.2f')).rjust(5))

    print("xCRV"," "*27,"cvxCRV minted","Ç"+str(myarray[-1]["cvx_rewards"][0]),end=" ")
    print(" "*2,Style.DIM+Fore.CYAN+str(round((myarray[-1]["cvx_rewards"][3]*(myarray[-1]["USD3pool"]/myarray[-1]["USD"]))+(myarray[-1]["cvx_rewards"][2]*(myarray[-1]["USDcvx"]/myarray[-1]["USD"]))+myarray[-1]["cvx_rewards"][1],2)),Style.RESET_ALL,end=" ")
    print(" "*9,"v"+str(format(round(myarray[-1]["cvx_rewards"][1],2), '.2f')).rjust(5)+"x"+str(format(round(myarray[-1]["cvx_rewards"][2],2), '.2f')).rjust(5)+"t"+str(format(round(myarray[-1]["cvx_rewards"][3],2), '.2f')).rjust(5))
...
def cvx_getvalue(printit, myarray):
    try:
        ...
        #if printit:
        print("Web3 crx call() failed, using previous values")
...
def trix_getvalue(printit, myarray):
    try:
        ...
        print("Web3 trix call() failed, using previous values")
