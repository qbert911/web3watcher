...
def print_status_line(carray, myarray, myarrayh, USD, eoa, w3, lookback):
    ...
    difference_afterparty = (diffc + diffd + diffe) / (60+extramins)*60
    ...
    return round(((difference*myarray[-1]["USD"])+(tprofit/60*60))*24*365/(sum(carray["invested"])+(myarray[-1]["trix_rewards"][0]*tripool_value_modifyer*carray["token_value_modifyer"][carray["longname"].index("tRicrypto")]))*100, 2), tripool_value_modifyer #display_percent
