...
def show_me(inputs, inpute, myarrayh,flag_tripped = 0):
    ...
    try:
        diffcvx = max(0,myarrayh[inputs]["cvxeth_rewards"][2]-myarrayh[inpute]["cvxeth_rewards"][2]) +\
                ...
                max(0,myarrayh[inputs]["crvsquared_extracvx"]-myarrayh[inpute]["cvxeth_extracvx"])+\
    ...
    try:
        ...
        ctr_unclaimed_past = ((1/acrv_coefficient)*211 / myarrayh[inpute]["concentrator_cvxeth_rewards"][0] *(myarrayh[inpute]["concentrator_cvxeth_rewards"][1]\
        ...
        concentrator_daily = (ctr_unclaimed_current-ctr_unclaimed_past\
                    +myarrayh[inputs]['concentrator_rewards_CTR']-myarrayh[inpute]['concentrator_rewards_CTR'])*acrv_coefficient

        daily_total = ((concentrator_daily) + cvxeth_daily + crvsquared_daily + crvstaked_daily + spelleth_daily + fxslocked_daily)*myarrayh[inputs]['USD']
    ...
    requirements_met = diffcrv > 4 and diffcvx > .5 and cvxeth_count > 30 and round(hours_elapsed/24, 2) == 1
    if requirements_met or flag_tripped:
        if (len(sys.argv) -1) or requirements_met:    
            ...
            dbuffer+=Fore.RED+"r"+Fore.WHITE+format((round(concentrator_daily, 2)), '.0f').rjust(2)
            ...
            difference =  round(mainpercentdisplay - daily_total)
        ...
        list_out = [diff3pool,diffcrv,concentrator_daily,diffcvx,totalvalue,cvxeth_count,daily_total,mainpercentdisplay]
