...
def header_display():
    ...
    for i in range(0, len(carray["name"])):
        ...
        print(carray["longname"][i].ljust(len(max(carray["longname"], key=len))), carray["name"][i], str(carray["invested"][i]).rjust(cw[0]),
              str(format(carray["virtprice"][i]*carray["balanceof"][i], '.2f')).rjust(cw[1]), str(format(carray["totalsupply"][i], '.0f')).rjust(cw[2]),
              Style.DIM+str(format(round(carray["minted"][i]/10**18, 2), '.2f')).rjust(cw[3])+Style.RESET_ALL, end=' ')
        ...
        if carray["currentboost"][i] > 0:
            ...
            print(Style.DIM+str(format(round(myarray[-1][carray["name"][i]+"pool"]-(carray["minted"][i]/10**18), 2), '.2f')).rjust(cw[4])+Style.RESET_ALL,
            ...
            print(Style.DIM+str(format(round(0, 2), '.2f')).rjust(cw[4])+Style.RESET_ALL,
...
def print_status_line(USD, eoa):
    ...
    print("\rAt $"+Fore.YELLOW+str(format(USD, '.3f'))+Fore.WHITE+" per CRV = "+
          Fore.GREEN+Style.BRIGHT+str(format(round((myarray[-1]["claim"]-myarray[eoa]["claim"])*USD*24*365/sum(carray["invested"])*100, 2), '.2f'))+Style.NORMAL+Fore.WHITE+"/"+
          Fore.CYAN+str(format(round((myarray[-1]["claim"]-myarray[eoa]["claim"])*24*365/sum(carray["invested"])*100, 2), '.2f'))+Fore.WHITE+"% APR", end=' ')

    for i in range(0, len(carray["name"])):
        if carray["currentboost"][i] > 0:
            print(Fore.RED+Style.BRIGHT+carray["name"][i]+Style.RESET_ALL+
                  str(format(round((myarray[-1][carray["name"][i]+"pool"]-myarray[eoa][carray["name"][i]+"pool"])*USD*24*365/carray["invested"][i]*100, 2), '.2f')), end=' ')

    print("- A"+csym+format(myarray[-1]["claim"], '.4f')+Style.RESET_ALL,
          "H"+csym+format((round(myarray[-1]["claim"]-myarray[eoa]["claim"], 4)), '.4f')+Style.RESET_ALL,
          "D"+csym+format((round((myarray[-1]["claim"]-myarray[eoa]["claim"])*24, 2)), '.2f')+Style.RESET_ALL,
          "Y"+csym+format((round((myarray[-1]["claim"]-myarray[eoa]["claim"])*24*365, 0)), '.0f')+Style.RESET_ALL, end=' - ')
...
def main():
    ...
    while True:                                                                     #Initiate main program loop
        ...
        for i in range(0, len(carray["name"])):
            ...
            if myarray[-1][carray["name"][i]+"pool"] - mydict[carray["name"][i]+"pool"] > 0: #debug lines ... should not happen
        ...
        if minut == "00" and mydict["claim"] > 1:
            ...
            time.sleep(3)
            show_me(-1, -2, 1, mydict["USD"], 1, 0, 0) #compare last record with 2nd to last, update price, do NOT end line
            print(" - ", end='')
            boost_check("                "+('\b'*12)+"\n")
        ...
        curve_hats_update(round((myarray[-1]["claim"]-myarray[eoa]["claim"])*myarray[-1]["USD"]*24*365/sum(carray["invested"])*100, 2),
