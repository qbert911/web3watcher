...
def convex_header_display(myarray):
    ...
    fxslocked_token_price = 2 * myarray[-1]["fxslocked_virt"] * ((myarray[-1]["FRAX"] * myarray[-1]["FRAX"])**(1/2)) #hack
...
#    print(f"({spelleth_imploss:3.0f}%/{min(days_of_imp_spell,999):3.0f}d)".ljust(11),end="")
    ...
    print(" "*52,"v"+str(format(round(myarray[-1]["fxslocked_rewards"][1],2), '5.2f')).rjust(5)+"      x"+str(format(round(myarray[-1]["fxslocked_rewards"][2]+myarray[-1]["fxslocked_extracvx"],2), '5.2f')).rjust(5)+"f"+str(format(round(myarray[-1]["fxslocked_extrafxs"],2), '5.2f')).rjust(5))
...
#    print(f"({cvxeth_imploss:3.0f}%/{min(days_of_imp_cvx,999):3.0f}d)".ljust(11),end="")
    ...
    print(" "*147,"t"+str(format(round(myarray[-1]["crvstaked_rewards"][3],2), '5.2f')).rjust(5)+"v"+str(format(round(myarray[-1]["crvstaked_rewards"][1],2), '5.2f')).rjust(5)+"x"+str(format(round(myarray[-1]["crvstaked_rewards"][2],2), '5.2f')).rjust(5))
    ...
    print(" "*169,"vv"+str(format(round(myarray[-1]["cvxlocked_rewards"][1],2), '5.2f')).rjust(5))
...
def combined_stats_display(myarray, carray, w3):
    ...
    cvxlocked_token = load_contract("0x62B9c7356A2Dc64a1969e19C23e4f579F9810Aa7",w3)
    ...
    cvxlocked_inwallet = round(call_me(cvxlocked_token.balanceOf(MY_WALLET_ADDRESS))/10**18)
    crv3pool_inwallet = round(call_me(crv3pool_token.balanceOf(MY_WALLET_ADDRESS))/10**18)

    fxslocked_inwallet = round(call_me(fxslocked_token.balanceOf(MY_WALLET_ADDRESS))/10**18,6)
    ...
    print(Fore.RED+"x"+Fore.WHITE+str(round(myarray[-1]["crvstaked_rewards"][2]+myarray[-1]["trix_rewards"][2]+myarray[-1]["mimx_rewards"][2]+myarray[-1]["crveth_rewards"][2])+round(myarray[-1]["cvxeth_rewards"][2])+round(myarray[-1]["spelleth_rewards"][2])+round(myarray[-1]["fxslocked_rewards"][2])+round(myarray[-1]["fxslocked_extracvx"])),end=" ")
    ...
    show_wallet(CRV_inwallet, cvxlocked_inwallet, cvx_inwallet, crv3pool_inwallet,fxslocked_inwallet,fxs_inwallet)
...
def show_wallet(CRV_inwallet,cvxlocked_inwallet,cvx_inwallet,crv3pool_inwallet,fxslocked_inwallet,fxs_inwallet):
    print("[", end='')
    if CRV_inwallet > 0:
        print("v"+f"{CRV_inwallet:02}"+Style.RESET_ALL, end='')
    if cvxlocked_inwallet > 0:
        print("vv"+f"{cvxlocked_inwallet:02}"+Style.RESET_ALL, end='')
    if cvx_inwallet > 0:
    ...
    if fxslocked_inwallet > 0:
        print("ff"+Fore.BLUE+f"{fxslocked_inwallet:02}"+Style.RESET_ALL, end='')
    if fxs_inwallet > 0:
