...
def convex_header_display(myarray):
    ...
    cvxfxs_value = ((myarray[-1]["cvxfxs_rewards"][2]*myarray[-1]["USDcvx"])+
                  (myarray[-1]["cvxfxs_rewards"][1]*myarray[-1]["USD"])+
                  (myarray[-1]["cvxfxs_extracvx"]*myarray[-1]["USDcvx"])+
                  (myarray[-1]["cvxfxs_extrafxs"]*myarray[-1]["FRAX"]))
    ...
    cvxfxs_token_price = 2 * myarray[-1]["cvxfxs_virt"] * ((myarray[-1]["FRAX"] * myarray[-1]["FRAX"])**(1/2)) #hack
    ...
    crveth_daily = max(0.0001,myarray[-1]["crveth_rewards"][1]-myarray[eoa]["crveth_rewards"][1]\
                ...
                / (60+extramins)*60)
    cvxfxs_daily = max(0.0001,myarray[-1]["cvxfxs_rewards"][1]-myarray[eoa]["cvxfxs_rewards"][1]\
                +((myarray[-1]["cvxfxs_rewards"][2]-myarray[eoa]["cvxfxs_rewards"][2])*myarray[-1]["USDcvx"]/myarray[-1]["USD"])\
                +((myarray[-1]["cvxfxs_extracvx"]-myarray[eoa]["cvxfxs_extracvx"])*myarray[-1]["USDcvx"]/myarray[-1]["USD"])\
                +((myarray[-1]["cvxfxs_extrafxs"]-myarray[eoa]["cvxfxs_extrafxs"])*myarray[-1]["FRAX"]/myarray[-1]["USD"]))\
                / (60+extramins)*60
    ...
    days_of_imp_cvx =  ( ((1884*myarray[-1]["USDcvx"])+((5.714+14)*myarray[-1]["ETH"])) - (myarray[-1]["cvxeth_rewards"][0]*cvxeth_token_price) ) / (cvxeth_daily *24*myarray[-1]['USD'])
    days_of_imp_spell = (6400 - (myarray[-1]["spelleth_rewards"][0]*spelleth_token_price) ) / (difference_spelleth *24*myarray[-1]['USD'])
    days_of_imp_fxs = ((4500*2) - (myarray[-1]["cvxfxs_rewards"][0]*cvxfxs_token_price))/ (cvxfxs_daily *24*myarray[-1]['USD'])
...
#    print(f"({spelleth_imploss:3.0f}%/{min(days_of_imp_spell,999):3.0f}d)".ljust(11),end="")
    ...
    print(" "*24,"v"+str(format(round(myarray[-1]["spelleth_rewards"][1],2), '5.2f')).rjust(5)+"x"+str(format(round(myarray[-1]["spelleth_rewards"][2],2), '5.2f')).rjust(5))

    print("cvxFXS "," "*5,Style.DIM+str(round(myarray[-1]["cvxfxs_rewards"][0]*cvxfxs_token_price)).rjust(6)+Style.RESET_ALL,end=" ")
    if days_of_imp_fxs > 0:
        print(f"({min(days_of_imp_fxs,999):3.0f}d)".ljust(7),end="")
    else:
        print(f"( ok )".ljust(7),end="")
    print(" "*1,"xf"+str(round(myarray[-1]["cvxfxs_rewards"][0]*2)).rjust(4),end=" ")
    print(" "*7,("$"+str(format(round(cvxfxs_value,2), '7.2f'))).rjust(8), end=" ")
    print(" "*52,"v"+str(format(round(myarray[-1]["cvxfxs_rewards"][1],2), '5.2f')).rjust(5)+"      x"+str(format(round(myarray[-1]["cvxfxs_rewards"][2]+myarray[-1]["cvxfxs_extracvx"],2), '5.2f')).rjust(5)+"f"+str(format(round(myarray[-1]["cvxfxs_extrafxs"],2), '5.2f')).rjust(5))

    print("xcrv-eth"," "*5,Style.DIM+str(round(myarray[-1]["crveth_rewards"][0]*crveth_token_price)).rjust(5)+Style.RESET_ALL,end=" ")
    if crveth_imploss > 0:
 ...
 #       print(f"({crveth_imploss:3.0f}%/{min(days_of_imp_crv,999):3.0f}d)".rjust(11),end=" ")
    ...
    print(" "*93,"v"+str(format(round(myarray[-1]["crveth_rewards"][1],2), '5.2f')).rjust(5)+"x"+str(format(round(myarray[-1]["crveth_rewards"][2],2), '5.2f')).rjust(5))

    print("xCVX/ETH"," "*4,Style.DIM+str(round(myarray[-1]["cvxeth_rewards"][0]*cvxeth_token_price)).rjust(6)+Style.RESET_ALL,end=" ")
#    print(f"({cvxeth_imploss:3.0f}%/{min(days_of_imp_cvx,999):3.0f}d)".ljust(11),end="")
    ...
    print(" "*123,"v"+str(format(round(myarray[-1]["cvxeth_rewards"][1],2), '5.2f')).rjust(5)+"x"+str(format(round(myarray[-1]["cvxeth_rewards"][2],2), '5.2f')).rjust(5))
    #print(" "*5,f"[{cvxeth_implossa}% {cvxeth_implossb}% {cvxeth_implossaraw} {cvxeth_implossbraw}]")

    #print("Purchased FXS "," "*13,Style.DIM+str(round(206*myarray[-1]["FRAX"])).rjust(5)+Style.RESET_ALL,end=" ")
    #print(" "*18,"f"+str(round(140+66)).rjust(4),end=" ")
    #print(" "*12,("$"+str(format(0, '7.2f'))).rjust(8))
    #66 lp token purchased
    #33 lp token purchased

    #print("Farmed FXS "," "*16,Style.DIM+str(round((198)*myarray[-1]["FRAX"])).rjust(5)+Style.RESET_ALL,end=" ")
    #print(" "*18,"f"+str(round((176+22))).rjust(4),end=" ")
    #print(" "*12,("$"+str(format(0, '7.2f'))).rjust(8))

    print("Staked CRV"," "*3,Style.DIM+str(round(myarray[-1]["cvx_rewards"][0]*myarray[-1]["USDcvxCRV"])).rjust(5)+Style.RESET_ALL,end="")
    print(" "*9,"vv"+str(myarray[-1]["cvx_rewards"][0]),end="")
    print(" "*8,("$"+str(format(round(cvx_value,2), '7.2f'))).rjust(8),end="")
    print(" "*147,"t"+str(format(round(myarray[-1]["cvx_rewards"][3],2), '5.2f')).rjust(5)+"v"+str(format(round(myarray[-1]["cvx_rewards"][1],2), '5.2f')).rjust(5)+"x"+str(format(round(myarray[-1]["cvx_rewards"][2],2), '5.2f')).rjust(5))

    print("Locked CVX"," "*3,Style.DIM+str(round(myarray[-1]["cvxcrv_rewards"][0]*myarray[-1]["USDcvx"])).rjust(5)+Style.RESET_ALL,end="")
    print(" "*10,"x"+str(round(myarray[-1]["cvxcrv_rewards"][0])).rjust(4),end="")
    print(" "*8,("$"+str(format(round(cvxcrv_value,2), '7.2f'))).rjust(8),end="")
    print(" "*169,"vv"+str(format(round(myarray[-1]["cvxcrv_rewards"][1],2), '5.2f')).rjust(5))
...
def curve_header_display2(myarray, carray, w3, fullheader):
    """display detailed pool information"""
    for i in range(0, len(carray["name"])):
        if carray["currentboost"][i] > 0 or fullheader:
            carray["totalsupply"][i] = round(call_me(load_contract(carray["gaugeaddress"][i],w3).totalSupply())/10**18, 2)
            ...
            if abs(round(myarray[-1][carray["name"][i]+"pool"]-round(carray["minted"][i]/10**18,2), 2)) > 0.05:
                ...
                print(" "*2,str("v"+format(round(myarray[-1][carray["name"][i]+"pool"]-(round(carray["minted"][i]/10**18,2)), 2), '5.2f')).rjust(6)+Style.RESET_ALL, end=' ')
...
def combined_stats_display(myarray, carray, w3):
    ...
    cvxfxs_token = load_contract("0xFEEf77d3f69374f66429C91d732A244f074bdf74",w3)
    ...
    cvxfxs_inwallet = round(call_me(cvxfxs_token.balanceOf(MY_WALLET_ADDRESS))/10**18,6)
    fxs_inwallet = round(call_me(fxs_token.balanceOf(MY_WALLET_ADDRESS))/10**18,6)


    #TODO add fxs (and votium if possible)
    ...
    x_claimable = (myarray[-1]["cvx_rewards"][3]*(myarray[-1]["USD3pool"]/myarray[-1]["USD"]))+\
                 ...
                 ((myarray[-1]["cvxfxs_rewards"][2]*myarray[-1]["USDcvx"]/myarray[-1]["USD"])+
                  (myarray[-1]["cvxfxs_rewards"][1]*myarray[-1]["USD"])+
                  (myarray[-1]["cvxfxs_extracvx"]*myarray[-1]["USDcvx"]/myarray[-1]["USD"])+
                  (myarray[-1]["cvxfxs_extrafxs"]*myarray[-1]["FRAX"])/myarray[-1]["USD"])
    ...
    print(Style.DIM+" {"+Fore.RED+"v"+Fore.WHITE+str(round(myarray[-1]["claim"]-(sum(carray["minted"])/10**18))+round(myarray[-1]["cvx_rewards"][1]+myarray[-1]["spelleth_rewards"][1]+myarray[-1]["trix_rewards"][1]+myarray[-1]["mimx_rewards"][1]+myarray[-1]["crveth_rewards"][1]+myarray[-1]["cvxeth_rewards"][1]+myarray[-1]["cvxcrv_rewards"][1]+myarray[-1]["cvxfxs_rewards"][1])),end=" ")
    print(Fore.RED+"x"+Fore.WHITE+str(round(myarray[-1]["cvx_rewards"][2]+myarray[-1]["trix_rewards"][2]+myarray[-1]["mimx_rewards"][2]+myarray[-1]["crveth_rewards"][2])+round(myarray[-1]["cvxeth_rewards"][2])+round(myarray[-1]["spelleth_rewards"][2])+round(myarray[-1]["cvxfxs_rewards"][2])+round(myarray[-1]["cvxfxs_extracvx"])),end=" ")
    print(Fore.RED+"t"+Fore.WHITE+str(round(myarray[-1]["cvx_rewards"][3])),end=' ')
    print(Fore.RED+"f"+Fore.WHITE+str(round(myarray[-1]["cvxfxs_extrafxs"]))+"}"+Style.RESET_ALL+" to claim     ",end=' ')
    show_wallet(CRV_inwallet, cvxcrv_inwallet, cvx_inwallet, crv3pool_inwallet,cvxfxs_inwallet,fxs_inwallet)
...
def show_wallet(CRV_inwallet,cvxcrv_inwallet,cvx_inwallet,crv3pool_inwallet,cvxfxs_inwallet,fxs_inwallet):
    print("[", end='')
    if CRV_inwallet > 0:
        print("v"+f"{CRV_inwallet:02}"+Style.RESET_ALL, end='')
    if cvxcrv_inwallet > 0:
        print("vv"+f"{cvxcrv_inwallet:02}"+Style.RESET_ALL, end='')
    if cvx_inwallet > 0:
    ...
    if cvxfxs_inwallet > 0:
        print("ff"+Fore.BLUE+f"{cvxfxs_inwallet:02}"+Style.RESET_ALL, end='')
    if fxs_inwallet > 0:
        print("f"+Fore.BLUE+f"{fxs_inwallet:02}"+Style.RESET_ALL, end='')
